...
class SearchTC(object):
  def search(self, task, entry, config=None):
    url = "https://tehconnection.eu/torrents.php?action=advanced&torrentname=" + entry["imdb_id"];
    ...
    for row in soup.find_all("tr", class_="group_torrent"):
      ...
      result = Entry();
      result["title"] = entry["title"] + " / " + info
      result["imdb_id"] = entry["imdb_id"]
...
def register_plugin():
  plugin.register(SearchTC, 'search_tehconnection', groups=['search'], api_ver=2)
